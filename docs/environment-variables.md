...
| 分类 | 变量 | 用途 |
|---|---|---|
| 运行时 | `NODE_ENV`, `PORT`, `YUNO_ROLE` | 生产模式、安全策略、HTTP 监听和进程职责 |
...
生产环境使用 `gemini-3.5-flash` 作为主回复模型，并使用 Google AI Studio 的 `gemini-3.1-flash-lite` 作为备用模型。主模型返回 429、超时、5xx 或不完整 JSON 前导语时，工作流会把同一份会话历史、系统提示和用户消息交给备用模型继续处理，不会先向用户发送中间错误提示。备用模型固定使用 `minimal` 推理并至少保留 384 个输出 tokens，避免短回复预算被内部推理耗尽；断路器按 provider 与模型隔离。
...
| 模型容错 | `REPLY_LLM_FALLBACK_CHAT_MODEL`, `MODEL_FALLBACK_CHAT_MODEL`, `REPLY_PRIMARY_TIMEOUT_MS`, `REPLY_HARD_TIMEOUT_MS`, `REPLY_TIME_BUDGET_MS`, `MODEL_CIRCUIT_FAILURE_THRESHOLD`, `MODEL_CIRCUIT_OPEN_MS`, `RETRY_ATTEMPTS`, `RETRY_DELAY_MS` |
...
| 队列 | `ENABLE_QUEUE`, `REDIS_URL`, `REPLY_QUEUE_NAME`, `PERSIST_QUEUE_NAME`, `QUEUE_RETRY_ATTEMPTS`, `QUEUE_BACKOFF_MS`, `QUEUE_CONNECT_TIMEOUT_MS`, `QUEUE_CONCURRENCY_DEFAULT`, `QUEUE_CONCURRENCY_REPLY`, `QUEUE_CONCURRENCY_PERSIST` |
| 自动化 | `AUTOMATION_TASK_CONCURRENCY`, `SCHEDULER_TASK_LOCK_MS`, `MAX_ACTIVE_REMINDERS_PER_USER`, `MAX_ACTIVE_SUBSCRIPTIONS_PER_USER`, `GROUP_EVENT_RETENTION_COUNT` |
...
`YUNO_ROLE` 支持 `all`、`api`、`onebot-ingress`、`reply-worker`、`persist-worker` 和 `scheduler`。

- `all`：默认单进程模式；Redis 未启用时允许 inline 队列。
- `api`：只开放 capture API，不要求 `NAPCAT_API`。
- `onebot-ingress`：只开放 OneBot 入站，必须使用 Redis/BullMQ，不要求 `NAPCAT_API`。
- `reply-worker`：消费 reply queue，要求模型、MongoDB、Redis 和 `NAPCAT_API`。
- `persist-worker`：消费 persist queue，只要求 MongoDB 和 Redis，不要求模型或 `NAPCAT_API`。
- `scheduler`：执行定时任务并直接发送，要求模型、MongoDB 和 `NAPCAT_API`。

拆分部署时，OneBot 入站产生的自动化结果也进入 reply queue，由 `reply-worker` 统一执行发送与 Delivery Ledger 去重。
