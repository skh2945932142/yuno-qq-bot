...
function inferLegacyIntent(text, rule = {}) {
  ...
  if (rule.keyword || /(help|帮助|帮我|问题|怎么|如何|为什么|为啥|能不能|会不会|是不是)/i.test(normalized)) return 'help';
...
function buildRuleSignals(event, context, options = {}) {
  ...
  const keyword = /(帮助|命令|问题|状态|关系|好感|画像|群状态|情绪|help|command|question|setting)/i.test(normalized);
